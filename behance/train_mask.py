...
# tag to save the trained model
tag = (
    "gan-mask_%s_%s_%d_%s_%s_%s_%s_c%.3f_a%.3f_p%.3f_g%.3f_cw%.3f_%s%.4f_%s%.4f_p%.1f_wd%.1f_dp%.2f_ep%d_%d_mb%d_%d_gr%.1f_%d_st%s_cnt%s_%s_%s"
    % (
...
(
    ...
    st_testset,
    st_nclass,
) = ld.load_dataset(args)
cnt_trainloader = th.utils.data.DataLoader(
    cnt_trainset,
...
cnt_testloader = th.utils.data.DataLoader(
    cnt_testset,
...
st_trainloader = th.utils.data.DataLoader(
    st_trainset,
    batch_size=args.batch_size,
    shuffle=True,
    num_workers=args.num_workers,
...
st_testloader = th.utils.data.DataLoader(
    st_testset,
    batch_size=args.test_batch_size,
    shuffle=True,
    num_workers=args.num_workers,
...
# ========================== autoencoder freezing/unfreezing ===============================
...
# combine generator, discriminator and loss to parallel. The main class which performs the training on the given dataset
class ae_gan(nn.Module):
    def __init__(self, args):
        ...
        # initializing the masked autoencoder
        net = mask_autoencoder(
            args.ae_flag,
            args.ae_dep,
            args.ae_mix,
            args.dropout,
            ...
            base_dep=base_dep,
            blur_flag=args.blur_perc,
        )
        net.load_model(args.enc_model, args.dec_model)

        # load pre-trained mask layers
        if (
            args.dise_model is not None
            and args.dise_model != "none"
            and args.dise_model != "None"
        ):
            net.load_dise_model(args.dise_model)

        # load pre-trained discriminator layers
        dnet = PatchDiscriminator(
            scn=st_nclass,
            ccn=cnt_nclass,
            n_layers=args.disc_dep,
            use_proj=args.use_proj,
            use_cnt=False,
        )
        if (
            args.disc_model is not None
            and args.disc_model != "none"
            and args.disc_model != "None"
        ):
    ...
    def train_disc(
        ...
        bc_out12, cc_out12, sc_out12 = dnet(
            img12.detach(), slabel=st_labels, clabel=labels
        )
    ...
    def forward(self, inputs, labels, st_inputs, st_labels, args):
        ...
        # autoencoder forward, reconstruct and exchange, probably can comment to save one forward pass
        img12, ftr1, grm2, aux12 = net(inputs, st_inputs)

        # discriminator for classification
        bc_out12, _, sc_out12 = dnet(img12, slabel=st_labels, clabel=labels)
        ...
        if args.per_w > 0 or args.gram_w > 0 or args.cycle_w > 0:
            base12, ftr12, grm12 = net.get_base_perc_gram(
                img12, blur_flag=args.blur_perc
            )
...
if args.train_dec:
    optimizer = mko.get_optimizer_var(
        [
            {
                "params": wrap_net.net.senc.parameters(),
                "params": wrap_net.net.dec.parameters(),
            }
        ],
        args,
        args.g_optm,
    ...
    optimizer = mko.get_optimizer_var(
        [{"params": wrap_net.senc.parameters()}], args, args.g_optm, args.lr
    )
doptimizer = mko.get_optimizer_var(
    wrap_net.dnet.parameters(), args, args.optm, args.d_lr
)

print((optimizer, doptimizer))


# saving th etrained model as a dictionary
...
def train(epoch):
    ...
    for bi, (inputs, labels) in enumerate(
        cnt_trainloader
    ):  # go through all content images
        if not os.path.exists(
            debug_folder
        ):  # break if the debug image folder is deleted
            print(("GAN training break at mb", bi))
            break
        try:
            st_inputs, st_labels = next(st_iter)
        ...
        if inputs.size() != st_inputs.size():
            print(
                (
                    "skip mb",
                    bi,
                    "autoencoder training: size does not match",
                    inputs.size(),
                    st_inputs.size(),
                )
            )
        ...
        if (
            (epoch > 1 or bi > 0)
            and args.optm == "padam"
            and (args.cla_w > 0 or args.gan_w > 0)
        ):
            doptimizer.pred_restore()  # restore weight for prediction metod

        # discriminator accuracy
        if len(gids) > 1:
            loss, running_dcl, running_dbl = wrap_net.module.train_disc(
                inputs,
                labels,
                st_inputs,
                st_labels,
            ...
            loss, running_dcl, running_dbl = wrap_net.train_disc(
                inputs,
                labels,
                st_inputs,
                st_labels,
        ...
        if bi % args.print_freq == 1:
            print(
                (
                    "training epoch: %d, minibatch: %d, dis class loss: %.3f, dis binary loss: %.3f"
                    % (epoch, bi, running_dcl / (bi + 1), running_dbl / (bi + 1))
                )
            )
            if len(gids) > 1:
                print(
                    (
                        "ep%d mb%d, discriminator loss details: " % (epoch, bi),
                        wrap_net.module.criterion.ls,
                    )
                )
            else:
                print(
        ...
        for p in tmp.dnet.parameters():
            p.requires_grad = False  # dont update discriminator
        unfreeze_generator(tmp.net)  # update generator
        ...
        if (bi % args.print_freq == 1) or (epoch == args.epochs and bi < 30):
            print(
                (
                    "training epoch: %d, minibatch: %d, loss: %f,  total time/mb: %f ms, running time/mb: %f ms"
                    % (
                        epoch,
                        bi,
                        running_loss / (bi + 1),
                        running_time / (bi + 1) * 1000.0,
                        (running_time - loading_time) / (bi + 1) * 1000.0,
                    )
                )
            )
    ...
    return (
        running_loss / len(cnt_trainloader),
        running_time,
        loading_time,
        running_dcl / len(cnt_trainloader),
        running_dbl / len(cnt_trainloader),
    )
...
def test(epoch):
    ...
    for bi, (inputs, labels) in enumerate(
        ...
        try:
            st_inputs, st_labels = next(st_titer)
        except StopIteration:
            st_titer = iter(st_testloader)
            st_inputs, st_labels = next(st_titer)
        if inputs.size() != st_inputs.size():
            print(
                (
                    "autoencoder testing: size does not match",
                    inputs.size(),
                    st_inputs.size(),
                )
            )
...
if not args.test_flag:
    ...
    while epoch < args.epochs:
        ...
        print(
            (
                "**training epoch: %d, mb: %d * %d, G loss: %f, D loss: %f, total time/mb: %f ms, running time/mb: %f ms, total time/epoch: %f s,"
                % (
                    epoch,
                    args.batch_size,
                    len(cnt_trainloader),
                    tr_l,
                    tr_dcl + tr_dbl,
                    running_time / len(cnt_trainloader) * 1000.0,
                    (running_time - loading_time) / len(cnt_trainloader) * 1000.0,
                    running_time,
                )
            )
        )
        ...
        print(
            (
                "**validating epoch: %d, running time/mb: %f ms, total time/epoch: %f s"
