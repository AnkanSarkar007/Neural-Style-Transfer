    ...
    def pred_step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                state["actual_step"].addcdiv_(
                    exp_avg, denom, value=-step_size
                )  # actual step
                p.data.addcdiv_(
                    exp_avg, denom, value=-step_size * (1 + self.pred_gm)
                )  # predicitive step
...
# Fetch the optimizer based on user provided flags
...
def get_optimizer_var(var, args, flag, lr):  # For variables
    if flag == "adam":
        ...
        optimizer = optim.RMSprop(
            var, lr=lr, momentum=args.momentum, weight_decay=args.weight_decay
        )
    elif flag == "sgd":
        optimizer = optim.SGD(
            var, lr=lr, momentum=args.momentum, weight_decay=args.weight_decay
...
def adjust_learning_rate(optimizer, init_lr, args, epoch, flag="linear"):
    ...
    if flag == "segment":
        ...
        s = (
            epoch // args.lr_freq
        )  # starting from lr_freq, learning rate drop linearly to 0.1 for each lr_freq
